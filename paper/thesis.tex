...
All measurements were collected on the Snellius genoa partition (AMD EPYC 9654, 192 cores per node, 336~GiB RAM per node; Snellius hardware specifications from~\cite{gptnl_architecture}, Appendix~4.2). We ran the GPT-NL curation pipeline \cite{gptnl_architecture} implemented with Datatrove \cite{datatrove} across four corpora from the GPT-NL Public Corpus \cite{gptnl_corpus}:
...
Each size was run with multiple replicates at single-task (ntasks=1) and multi-task (ntasks=16, 32) configurations. The production curation pipeline \cite{gptnl_architecture} consists of six stages, of which we measured five. PII masking runs on the \texttt{gpu\_a100} partition using the PrivateAI service~\cite{gptnl_architecture} and was excluded from this study:
...
EAR was configured with the \texttt{monitoring} policy (as opposed to \texttt{minimization}, which actively adjusts CPU frequency). The monitoring policy records energy metrics without interfering with job execution. EAR produces two output files per node per job: a per-job aggregate file (\texttt{\_apps.csv}) containing total energy and average power, and a time-series file (\texttt{\_loops.csv}) with per-loop metrics. We use the aggregate files exclusively, summing \texttt{DC\_NODE\_POWER\_W} $\times$ \texttt{ELAPSED} across all rows to obtain per-stage energy in Joules. On AMD Genoa nodes, \texttt{DRAM\_POWER\_W} is not available---only total DC node power is recorded---so we work with node-level energy rather than component-level breakdowns.
